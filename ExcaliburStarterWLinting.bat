...
call npm i --save-dev eslint @typescript-eslint/parser @typescript-eslint/eslint-plugin @eslint/js
...
:: eslint.config.js
:: ************************************
echo creating eslint.config.js file in root
...
echo         ...Object.fromEntries^(Object.keys^(globals.browser^).map^(name ^=^> ^[name, "readonly"^]^)^), >>eslint.config.js
(
echo       ^},
echo     ^},
echo     plugins: ^{
...
echo     ^},
echo     rules: ^{
echo       // Require a break unless an explicit fallthrough comment is present
echo       "no-fallthrough": ["error", ^{ commentPattern: "falls?\\s?through" ^}],
echo     ^},
echo   ^},
echo ];
)>>eslint.config.js
...
(
    echo // resources.ts
    echo import ^{ ImageSource, Loader ^} from "excalibur";
...
call npx json -I -f package.json -e "this.scripts.lint='eslint \"**/*.{ts,tsx,js}\"'"
call npx json -I -f package.json -e "this.version='0.0.1'"
call npx json -I -f package.json -e "this.type='module'"
